# Documentação GWAN BFF

Bem-vindo à documentação do GWAN BFF (Backend for Frontend). Esta documentação está organizada em seções para facilitar a navegação e compreensão do projeto.

## 📚 Índice

### 🚀 Início Rápido
- [Instalação](getting-started/installation.md)
- [Configuração](getting-started/configuration.md)
- [Executando o Projeto](getting-started/running.md)

### 🏗️ Arquitetura
- [Visão Geral](architecture/overview.md)
- [Clean Architecture](architecture/clean-architecture.md)
- [Estrutura do Projeto](architecture/project-structure.md)

### 📡 API
- [Autenticação](api/authentication.md)
- [Chatbots](api/chatbots.md)
- [Dataset](api/dataset.md)
- [Knowledge Base](api/knowledge-base.md)
- [Profile](api/profile.md)
- [Users](api/users.md)

### 🔧 Desenvolvimento
- [Guia de Desenvolvimento](development/guide.md)
- [Padrões de Código](development/code-standards.md)
- [Testes](development/testing.md)

### 🚀 Deploy
- [Ambiente de Produção](deployment/production.md)
- [Docker](deployment/docker.md)
- [Monitoramento](deployment/monitoring.md)

### 📊 Monitoramento
- [Logs](monitoring/logs.md)
- [Métricas](monitoring/metrics.md)
- [Alertas](monitoring/alerts.md)

### 🔐 Segurança
- [Autenticação e Autorização](security/auth.md)
- [Configuração de Segurança](security/configuration.md)
- [Boas Práticas](security/best-practices.md)

### 🗄️ Armazenamento
- [MongoDB](storage/mongodb.md)
- [MinIO](storage/minio.md)
- [RabbitMQ](storage/rabbitmq.md)

### 🤖 Workers
- [Email Worker](workers/email-worker.md)
- [Chatbot Worker](workers/chatbot-worker.md)
- [Knowledge Worker](workers/knowledge-worker.md)

### 🔧 Troubleshooting
- [Problemas Comuns](troubleshooting/common-issues.md)
- [Guia de Depuração](troubleshooting/debugging.md)
- [FAQ](troubleshooting/faq.md)

## 📝 Contribuindo

Para contribuir com a documentação, por favor leia nosso [guia de contribuição](CONTRIBUTING.md).

## 📄 Licença

Esta documentação está sob a licença MIT. Veja o arquivo [LICENSE](LICENSE) para mais detalhes. 