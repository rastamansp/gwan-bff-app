...
import { UpdateN8nConfigDto, ChatbotResponseDto } from '../../domain/dtos/chatbot.dtos';
import { Chatbot } from '../../domain/entities/chatbot.entity';
import { CHATBOT_REPOSITORY } from '../../domain/tokens/injection.tokens';

@Injectable()
export class UpdateN8nConfigUseCase {
    constructor(
        @Inject(CHATBOT_REPOSITORY)
        private readonly chatbotRepository: IChatbotRepository
    ) { }

    async execute(userId: string, id: string, dto: UpdateN8nConfigDto): Promise<ChatbotResponseDto> {
        ...
        const updatedChatbot = await this.chatbotRepository.updateN8nConfig(id, {
            ...dto,
            updatedAt: new Date(),
        });
