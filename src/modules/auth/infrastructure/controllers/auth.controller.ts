...
export class AuthController {
  ...
  async register(
    ...
    this.logger.log(
      `[Register] Iniciando processo de registro para ${data.email}`,
    );
    this.logger.debug(
      `[Register] Dados recebidos: ${JSON.stringify({
        name: data.name,
        email: data.email,
    ...
    try {
      ...
      this.logger.log(
        `[Register] Registro concluído com sucesso para ${data.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `[Register] Erro no registro para ${data.email}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async verify(@Body() data: { email: string; code: string }) {
    this.logger.log(
    ...
    try {
      ...
      this.logger.log(
        `[Verify] Código verificado com sucesso para ${data.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `[Verify] Erro na verificação para ${data.email}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async login(@Body() data: { email: string }) {
    ...
    try {
      ...
      this.logger.log(
        `[Login] Código de login enviado com sucesso para ${data.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `[Login] Erro no login para ${data.email}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  @ApiResponse({ status: 400, description: "Invalid code" })
  async verifyLogin(@Body() data: { email: string; code: string }) {
    this.logger.log(
      `[VerifyLogin] Iniciando verificação de código de login para ${data.email}`,
    );
    try {
      const result = await this.verifyLoginUseCase.execute(data);
      this.logger.log(
        `[VerifyLogin] Código de login verificado com sucesso para ${data.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `[VerifyLogin] Erro na verificação de login para ${data.email}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
